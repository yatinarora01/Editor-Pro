...
ALLOWED_EXTENSIONS = {'png', 'pdf', 'jpg', 'jpeg', 'gif','webp'}
...
def edit():
    ...
    # check if the post request has the file part
        ...
        if file and allowed_file(file.filename):
            ...
            flash(f"Your image has been and availabe at <a href='/{new}' target ='_blank' >here</a>")
            return  render_template("index.html") #redirect(url_for('download_file', name=filename))
        
    
    return render_template("index.html")

app.run(debug=True, port=5001)  # sometimes when port is bu then we can
                      # give our port as app.run(debug=True,port=5001)
